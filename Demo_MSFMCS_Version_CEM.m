% This demo implements the MSFMCS using CEM as initial classifier for 
...
%%%% Initial module - SC 
% generate target spectral matrix of CEM
imgVec = ToVector(img);
gtVec = ToVector(gt);
target = zeros(no_classes,no_bands);
for i = 1:no_classes
    target(i,:) = mean(imgVec(find(gtVec==i),:));
end 

[ CEMaps, GSmaps, GsSfMaps ] = CEM( img, target );
[precision, recall, AA, OA, APR, OPR, confus_m] = confusion_matrix(gt,CEMaps);
SROA = [SROA OA];
...
SVMmaps = func_SVM( GsSfMaps, GroundT, indexes );
...
%%% spatial filter - gaussian filter
[clasMap, GsSfMaps] =  GSFilter(SVMmaps, no_classes);
